...
func (a *iKey) Compare(b avl.Entry) int {
...
func (db *DB) Delete(r ...iKV)  {
	l := len(r)
	k := make([]avl.Entry, l,l)
	for i := 0; i < l; i++ {
		d := r[i]
		k[i] = NewKey(d.k, d.v)
	}
	db.tr,_ =  db.tr.Delete(k...)
}

func (db *DB) Get(r ...iKV) avl.Entries {
	l := len(r)
	k := make([]avl.Entry, l,l)
	for i := 0; i < l; i++ {
		d := r[i]
